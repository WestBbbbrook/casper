...
void judge(int nownode, int nowquadnode){
	...
	if(Node[nownode].A<=4){
		...
		if(Quadnode[Quadnode[nowquadnode].fa].userct<=Node[nownode].K){
	    	...
	    	int num=nowquadnode%10;
	    	for(int j=0;j<4;++j){
		    	Node[nownode].CID[t].push_back(Quadnode[Quadnode[nowquadnode].fa].child[order[num][j]]);
		        nowK=nowK+Quadnode[Quadnode[Quadnode[nowquadnode].fa].child[order[num][j]]].userct;
		     	nowA+=4;
		    	//如果够了就返回，不够继续直到加完4个 
