...
void init_user(){
	...
	for(int i=1;i<=1000;++i){
		Node[i].K=random(1,150);
		Node[i].A=random(1,15);
	} 
}
...
void judge(int nownode, int nowquadnode){
	...
	if(Node[nownode].A<=4){
		...
		if(Quadnode[Quadnode[nowquadnode].fa].userct<=Node[nownode].K){
			...
			//把这四个几点的CID加入到该用户节点里 
			for(int i=1;i<=4;++i){
				Node[nownode].CID[t].push_back(Quadnode[Quadnode[nowquadnode].fa].child[i]);
			}
			//因为在四叉树上是从左往右，加完加4，好算 
			nowquadnode=nowquadnode+4; 
	    //果小于4的区域用户数够，那就一个一个加  ，看是不是都用不了4个 
	...
	//如果所需区域大于4，那就把当前的最小网格的4个网格的CID都算里 
	}else{
		for(int i=1;i<=4;++i){
			Node[nownode].CID[t].push_back(Quadnode[Quadnode[nowquadnode].fa].child[i]);
		}
	...
	//然后 从当前网格按照每层1234的顺序依次加最小网格 ，直到A和K都满足要求 
	for(int i=nowquadnode;i<=444;++i){
		if(Node[nownode].A<=nowA&&Node[nownode].K<=nowK){
			return;
		}else{
			if(Quadnode[i].fa!=0){
				nowA++;
				nowK=nowK+Quadnode[i].userct;
				Node[nownode].CID[t].push_back(i);
			}
	    }
	} 
	//如果不满足。就从111开始到当前节点在找，因为前面的节点刚才没加 
	for(int i=111;i<nowquadnode;++i){
		if(Node[nownode].A<=nowA&&Node[nownode].K<=nowK){
			return;
		}else{
			if(Quadnode[i].fa!=0){
...
void  adjust_cid(int nownode){
	//求当前用户节点的cid数目 
	int cidct=Node[nownode].CID[t].size();
	//int cid[cidct];
	//求这个用户的cid包括的每次网格数目 
	int grid1=0;
	int grid2=0;
	...
	grid2=cidct%16/4;
	grid3=cidct%16%4;
	//nowcid为当前网格，把他设置为他的父节点 
	int nowcid=Node[nownode].CID[t][cidct-1];
	nowcid=Quadnode[nowcid].fa;
	//因为是从左往右加的四叉树节点，我们从第三层的开始处理，所有要从右往左
	//所有nowcid-1为当前父节点的左兄弟节点
	//如果已经是末位1了，则+3，否则-1 
	if(nowcid%10!=1){
		nowcid=nowcid-1;
	} else{
	...
	for(int i=0;i<grid3;++i){
		//cid[i]=Node[nownode].CID[1][cidct-1];
		cid.push_back(Node[nownode].CID[t][cidct-1]);
		cidct--; 
	}
	//如果有，加入第二层的 
	for(int i=0;i<grid2;++i){
		cid.push_back(nowcid);
		if(nowcid%10!=1){
			nowcid--;
		}else {
			nowcid=nowcid+3;
...
int main(){
	...
	for(t=1;t<=100;++t){
        ...
        for(int n=1; n<=1000; ++n){
            ...
            ofstream out(strcid);
            out<<"n="<<n<<" A="<<Node[n].A<<" K="<<Node[n].K<<" CID=";
		...
		/*
    	for(int i=1;i<=3;++i){
    	    cout<<"i="<<i<<" A="<<Node[i].A<<" K="<<Node[i].K<<
		        //  " x="<<Node[i].rang[0].x<<" y="<<Node[i].rang[0].y<<
			     " CID=";
		    for(int j=0;j<Node[i].CID[t].size();++j){
		        cout<<" -"<<Node[i].CID[t][j];
	    	}
	    	cout<<"\n";
    	}   */
